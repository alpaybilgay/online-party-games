...
import kpssData from "../../data/kpss_vatandaslik_1000_soru.json";
...
function KpssVatandaslik({
  ...
  console.log("KPSS VATANDASLIK STATE UPDATE:", {
    socketId: socket?.id,
    hostId: room?.hostId,
    isMeHost,
    myInfo,
  ...
  const handleUpdateSettings = (cats, dur, autoAdv) => {
    ...
    if (socket && room && isMeHost) {
      socket.emit("kpss-vatandaslik-update-settings", {
        categories: cats,
        duration: dur,
        autoAdvance: autoAdv
      });
  ...
  const handleStartQuestion = () => {
    ...
    const askedIds = room?.askedQuestionIds?.["KPSS VATANDAŞLIK"] || [];
    ...
    socket.emit("kpss-vatandaslik-start-question", { question: randomQ });
  };
  ...
  const handleSubmitChoice = (choiceText) => {
    ...
    socket.emit("kpss-vatandaslik-submit-choice", { choice: choiceText }, (res) => {
      if (res.success) {
  ...
  const handleNextRound = () => {
    if (socket && room && isMeHost) {
      socket.emit("kpss-vatandaslik-next-round");
    }
  };

  const handleResetPool = () => {
    if (socket && room && isMeHost) {
      socket.emit("kpss-vatandaslik-reset-pool");
      setOutOfQuestions(false);
      setActiveError("");
    }
  };
      ...
      <div className="relative min-h-screen bg-[#070709] text-zinc-100 flex flex-col items-center justify-center font-sans px-4">
        ...
        <div className="w-full max-w-md z-10">
          ...
          <div className="bg-[#0e0e11]/80 border border-zinc-900 rounded-3xl p-6 shadow-2xl backdrop-blur-md">
            ...
            <button
              onClick={() => handleJoinGame("KPSS VATANDAŞLIK")}
          ...
          <div className="text-center mt-6">
            <button
              onClick={onLeave}
              className="text-xs font-bold text-zinc-505 hover:text-zinc-300 transition-colors cursor-pointer"
            >
  ...
  if (isInRoom && !gameState) {
    return (
      <div className="relative min-h-screen bg-[#070709] text-zinc-100 flex flex-col items-center justify-center font-sans">
        ...
        <span className="text-xs text-zinc-505 mt-3 font-semibold animate-pulse">Bağlanılıyor...</span>
      </div>
    );
  }
        ...
        <header className="relative z-10 border-b border-zinc-900/50 bg-[#09090b]/60 backdrop-blur-md">
          <div className="max-w-5xl mx-auto px-6 h-14 flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs font-bold text-zinc-400">KPSS Vatandaşlık Lobisi</span>
            </div>
            <button
              onClick={onLeave}
              ...
              <div className="bg-[#0e0e11]/60 border border-zinc-900 p-6 rounded-2xl backdrop-blur-sm space-y-6">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-[9px] text-zinc-505 font-semibold">
                ...
                {/* Duration */}
                <div className="space-y-3">
                  <label className="block text-[10px] font-bold text-zinc-400 tracking-widest uppercase text-left">Soru Başına Süre</label>
                ...
                {/* Transition mode */}
                <div className="space-y-3">
                  <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-widest text-left">Soru Geçiş Modu</label>
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    >
                      ...
                      <span className="text-[9px] text-zinc-505 mt-0.5 font-medium leading-tight">Cevaplama bitince 5 sn sonra sıradaki soruya geçilir.</span>
                    </button>
                    ...
                    >
                      ...
                      <span className="text-[9px] text-zinc-505 mt-0.5 font-medium leading-tight">Her sorudan sonra adminin butona basması istenir.</span>
                    </button>
                  </div>
                </div>
              ...
              <div className="bg-[#0e0e11]/60 border border-zinc-900 p-6 rounded-2xl backdrop-blur-sm flex flex-col items-center justify-center text-center space-y-5 min-h-[300px]">
                ...
                <div className="w-full max-w-xs bg-zinc-950/50 border border-zinc-900 rounded-xl p-3 text-left space-y-2">
                  <div className="flex justify-between text-[10px]">
                    <span className="text-zinc-505 font-semibold">Soru Süresi:</span>
                    <span className="text-zinc-300 font-bold">{selectedDuration} saniye</span>
                  </div>
                  <div className="flex justify-between text-[10px]">
                    <span className="text-zinc-505 font-semibold">Geçiş Modu:</span>
                  ...
                  <div className="flex flex-col text-[10px] pt-1.5 border-t border-zinc-900">
                    <span className="text-zinc-505 font-semibold mb-1">Seçilen Kategoriler:</span>
        ...
        <div className="w-full max-w-md bg-[#0e0e11] border border-zinc-900 rounded-3xl p-6 text-center space-y-6 shadow-2xl">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-2 leading-relaxed">
              Seçtiğiniz kategorilerdeki tüm vatandaşlık sorularını başarıyla tamamladınız.
            </p>
          </div>
        ...
        <header className="relative z-10 border-b border-zinc-900/60 bg-[#09090b]/80 backdrop-blur-md">
          <div className="max-w-5xl mx-auto px-4 h-12 flex items-center justify-between">
            <span className="text-[10px] font-extrabold uppercase px-2 py-0.5 rounded bg-zinc-900 border border-zinc-800 text-amber-500">
              {currentQuestion?.category || "Vatandaşlık"}
            </span>
            ...
            {!isResult ? (
              ...
              <div className="flex flex-col justify-between h-full space-y-5 animate-fade-in">
                <div>
                  ...
                  {/* Question Text */}
                  <h2 className="text-sm sm:text-base font-extrabold text-zinc-200 leading-relaxed mb-6 text-center select-none animate-fade-in">
                ...
                {/* Status Bar */}
                <div className="mt-5 pt-3 border-t border-zinc-900/80 flex items-center justify-between text-[9px] font-bold text-zinc-505 uppercase tracking-wider">
                  <span>
              ...
              <div className="flex flex-col justify-between h-full space-y-5 animate-fade-in">
                <div>
                  <div className="flex justify-between items-center mb-3 pb-2 border-b border-zinc-900/60">
                    ...
                    <span className="text-[9px] font-mono text-zinc-505">ID: #{currentQuestion?.id}</span>
                  </div>
                        ...
                        return (
                          <div key={p.id} className="flex justify-between items-center text-[10px]">
                            ...
                            {isCorrect ? (
                              ...
                              <span className="font-bold text-zinc-650">0 XP (Yanlış/Cevapsız)</span>
                            )}
                          </div>
                        );
                      })}
                  ...
                  <div className="pt-1.5 flex items-center justify-between border-t border-zinc-900/60">
                    <div>
                      {autoAdvance ? (
                        ...
                        <span className="text-[9px] font-semibold text-zinc-505 uppercase">
...
export default KpssVatandaslik;
